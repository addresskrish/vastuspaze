import aboutImage from '../assets/about.webp'

const About = () => {
  return (
    <section className="max-w-7xl mx-auto border-b-2 mt-20" id="about">
        <h2 className="text-xl lg:text-3xl text-center uppercase mb-12 mx-4">
            About
        </h2>
        <div className="flex flex-col items-center lg: space-x-8 mx-4 mb-20">
          <img src={aboutImage} alt="About VastuSpaze" className='w-full h-auto' />
          <p className='text-lg lg:text-3xl font-light text-center lg:text-left max-w-5xl mx-auto mt-8'>
            At VastuSpaze, we believe in transforming houses into dream homes.
            With a passon for design and a comitment to quality, our team of
            skilled professionals has been dedicated to delivering exceptional
            revolution services for over a decade. We specialize in creating
            beautiful, functional spaces that reflect out clients unique tastes
            and lifestyles. From modern kitchens and luxurius bathrooms to serene
            outdoor living areas and cozy home offices, we bring to out expertise to
            every project, ensuring each revolution is tailored to meet your
            specific needs and desires. Our mission is to enhance the beauty and 
            functionality of your home, making its a place where you can truly
            thrive.

          </p>
        </div>
    </section>
  )
}

export default About